...
class ModulesArgumentsHelper(interface.ArgumentsHelper):
  """Modules CLI arguments helper."""

  NAME = 'modules'
  DESCRIPTION = 'Modules command line arguments.'
  ...
  def AddArguments(cls, argument_group):
    ...
    argument_group.add_argument(
        '--modules', dest='modules', type=str, action='store',
        default='', metavar='MODULE_FILTER_EXPRESSION', help=(
  ...
  def ParseOptions(cls, options, configuration_object):
    ...
    modules = cls._ParseStringOption(options, 'modules', default_value='')
    modules = modules.replace('\\', '/')

    # TODO: validate parser names.

    setattr(configuration_object, '_module_filter_expression', modules)


manager.ArgumentHelperManager.RegisterHelper(ModulesArgumentsHelper)