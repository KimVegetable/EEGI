...
class XLSPlugin(interface.DEFAPlugin):
    NAME = "XLS"
    DESCRIPTION = 'xls plugin'

    def Process(self, **kwargs):
        super(XLSPlugin, self).Process(**kwargs)

        data = MappingDocuments()

        compound = Compound(kwargs['fp'])
        compound.fileType = "xls"
...
defa_connector.DEFAConnector.RegisterPlugin(XLSPlugin)
